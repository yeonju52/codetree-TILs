...
int main(int argc, char** argv)
{
	...
	for (test_case = 1; test_case <= T; ++test_case)
	{
		...
		while (true) {
			...
			// 2. 메두사의 시선
			// 상(0, 4, 5), 하(1, 6, 7) 좌(2, 4, 6), 우(1, 3, 5)
			vector<vector<int>> dir = { {0, 4, 5}, {1, 6, 7}, {2, 4, 6}, {3, 5, 7} };
			...
			for (int i = 0; i < 5; i++) { // 4 + 1(정한 find_d로)
				...
				for (pos &war : warrior) {
					...
					if (i == 0 && war.x < x) {
						if (war.y < y) view(war.x, war.y, 3, { 0, 4 });
						else if (war.y == y) view(war.x, war.y, 3, { 0 });
						else view(war.x, war.y, 3, { 0, 5 });
					}
					else if (i == 1 && war.x > x) {
						if (war.y < y) view(war.x, war.y, 3, { 1, 6 });
						else if (war.y == y) view(war.x, war.y, 3, { 1 });
						else view(war.x, war.y, 3, { 1, 7 });
					}
					else if (i == 2 && war.y < y) {
						if (war.x < x) view(war.x, war.y, 3, { 2, 4 });
						else if (war.x == x) view(war.x, war.y, 3, { 2 });
						else view(war.x, war.y, 3, { 2, 6 });
					}
					else if (i == 3 && war.y > y) {
						if (war.x < x) view(war.x, war.y, 3, { 3, 5 });
						else if (war.x == x) view(war.x, war.y, 3, { 3 });
						else view(war.x, war.y, 3, { 3, 7 });
					}
				}
				if (!real) {
					cnt = 0;
